...
try:
    ...
    for name,source in [('OvernightHarness.dll',Path(replay_source)/'build/OvernightHarness.dll' if replay_source and os.environ.get('DEALMAKER_SNAPSHOT_CAPTURE')!='1' and not os.environ.get('DEALMAKER_SNAPSHOT_PLAYBACK') else root/'tests/overnight/bin/Release/net9.0/OvernightHarness.dll'),('OvernightHarness.json',root/'tests/overnight/OvernightHarness.json')]:
